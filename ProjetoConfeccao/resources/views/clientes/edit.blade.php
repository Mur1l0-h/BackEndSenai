                    ...
                    <form action="{{ route('clientes.update', $cliente) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="reserva" id="reserva" required 
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-800 focus:ring-gray-800 sm:text-sm transition duration-150 ease-in-out">
                                <option value="" disabled {{ $cliente->reserva === null ? 'selected' : '' }}>Selecione uma opção</option>
                                <option value="1" {{ $cliente->reserva == '1' ? 'selected' : '' }}>Sim</option>
                                <option value="0" {{ $cliente->reserva == '0' ? 'selected' : '' }}>Não</option>
                            </select>
                            @error("reserva")
                                <span class="text-sm text-red-500 mt-1 block">{{ $message }}</span>
                            @enderror
